...
@tbl-balance-tables provide some information about the balance of project attributes pooled across all `r atp[, english(uniqueN(Project.Cycle))]` project cycles, including average values and two-sample t-statistics. @tbl-cutoff describes the balance of project attributes for all projects above and below the statewide funding threshold. @tbl-window also looks above and below the statewide funding cutoff, but only within a window of `r balance_windows[round == "Pooled", window]` points on either side of the threshold score. This window was chosen using a set of nested binomial tests for density balance on either side of the threshold score. Finally, @tbl-funded compares all unfunded projects to all funded projects, regardless of their score. 
...
#| tbl-subcap: 
#|   - All projects, above and below the statewide funding cutoff. {#tbl-cutoff}
#|   - Projects within a normalized score of ±2.75 of the statewide funding cutoff. {#tbl-window}
#|   - All projects, selected for funding or not selected for funding (in any funding component) {#tbl-funded}
...
As I get additional data on projects, as well as spatially merge baseline outcome and demographic data, I will expand this comparison of covariates, and also generate additional plots of covariates over the running variable across the funding threshold, to detect discontinuities in the covariates, again a sign of manipulation or selection into treatment. Covariates should vary continuously across the treatment threshold. I intend to pair these graphs with a joint test of the impact of the threshold on the covariates, using a Seemingly Unrelated Regression (SUR) and $\chi^2$ test [@lee2010 section 4.4.2], and may also pursue placebo tests, in accordance with @imbens2008.

@fig-treatment-prob shows the probability of receiving funding against the normalized running variable across the statewide funding threshold, separately for each funding cycle, as well as all projects pooled together. Because projects receive funding in both the statewide component and the regional component, projects below the statewide funding threshold still receive funding. Unsurprisingly, however, there is still a sizeable jump in funding probability at the statewide threshold, where it jumps to 100%[^unfunded]. In this research design, receiving funding is the instrument, or intention to treat. 
...
Once I have complete data on project construction status, I will be able to create graphs of _treatment_ probability along the running variable, where the treatment is the construction of an actual project. Because this research design is a fuzzy RD, there will still be some nonzero probability of treatment below the threshold, and units above the threshold may have treatment probability below 1. These graphs will show the magnitude in the jump in treatment probability across the threshold.

## Intended Specification and Estimation

Finally, I will present my main outcome plots, graphing the outcome variable(s) along the running variable, across the treatment threshold. These binned scatter plots will be accompanied by a quadratic fit polynomial on either side of the threshold (leaving out higher order terms, per @gelman2019). These plots should demonstrate the treatment effect (or lack thereof) of streetscape improvements on the outcome variables of interest.
